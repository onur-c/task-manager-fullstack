  ...
  return (
    <>
      <div className="mt-auto w-full relative ">
        <h1 className="ml-8 text-5xl font-extrabold">Completed Tasks</h1>
        <main className=" shadow-xl m-8  border border-border p-6  rounded flex flex-wrap gap-3 overflow-y-auto h-[calc(100vh-150px)] bg-background ">
          {taskQuery.isSuccess &&
          !!taskQuery.data.data.filter((task: TaskDataType) => task.isCompleted)
            .length ? (
            taskQuery.data.data
              .filter((task: TaskDataType) => task.isCompleted)
